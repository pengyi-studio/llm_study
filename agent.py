...
class travel_agent:
    def __init__(self, api_key=None, base_url=None, model=None, messages=None):
        ...
        if not messages:
            self.messages = [{
                "role":"system", "content":"你是一位名叫悠悠的AI旅行助手。"
            }]
        else:
    ...
    # 这个是深度分析任务的接口，实际上这两个接口不需要合并到一个函数中，因为这就是两个功能，在前端中完全可以用一个开关隔开
...
if __name__ == "__main__":
    ...
    response = travelAgent.research_task()
    print("response")